...
class Neo4jDataGenerator:
    ...
    def generar_datos_escala_pequena(self, num_usuarios=100, num_productos=50, max_pedidos_por_usuario=5):
        start_time = time.time()
        print(f"🚀 Comenzando generación de datos...")
        print(f"   👥 Usuarios: {num_usuarios}")
        print(f"   🛍️  Productos: {num_productos}")
    ...
    def _crear_pedido_completo(tx, order_id, user_id):
        ...
        for _ in range(num_productos):
            product_id = random.randint(1000, 1049)  # IDs de productos generados (50 productos)
...
if __name__ == "__main__":
    ...
    # PARÁMETROS DE ESCALA PEQUEÑA
    NUM_USUARIOS = 100      # <-- Ajustable
    NUM_PRODUCTOS = 50      # <-- Ajustable
